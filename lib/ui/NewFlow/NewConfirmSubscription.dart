...
class _HomeScreenState extends State<NewConfirmSubscription> {
  ...
  submit() async {
    ...
    holidaysList.forEach((element) => holidays.add(CommonUtils.getSimpleDateForApi(element)));

    ConfirmOrderRequestModel request = ConfirmOrderRequestModel(
        ...
        duration: selectionType == RangeSelectionType.Weekly
            ? "weekly"
            : selectionType == RangeSelectionType.Monthly
            ? "monthly"
            : "custom",
        ...
        preferredDeliveryTime: prefferedTime.toString());
    ...
    NetworkUtil().post(url: ApiConfig.userCheckStockAvailable, token: token, body: jsonEncode(request)).then((res) {
      ...
      if (checkOutResponse.status == 200) {
        CommonUtils.showToast(msg: "Meal added to cart", bgColor: Colors.black, textColor: Colors.white);
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => CartScreen(stockCheckOutResponse: checkOutResponse, confirmOrderRequestModel: request)));
      } else {
        CommonUtils.showToast(
