...
// Declare this variable
                                        ...
                                        child: ListTile(
                                          title: Text(
                                            ...
                                            style: Theme.of(context).textTheme.subtitle1.copyWith(
                                                color: index == snapshot.data.length ? Colors.black38 : Colors.black),
                                          ),
                                          leading: Radio(
                                            value: index,
                                            groupValue: _value,
                                            activeColor: Color(0xFF6200EE),
                                            onChanged: index == snapshot.data.length
                                                ? null
                                                : (int value) {
                                                    setState(() {
                                                      _value = value;
                                                      print("radioButton: $_value");
                                                    });
                                                  },
                                          ),
                                        ),
                            ...
                            child: RaisedButton(
                              ...
                              onPressed: () async {
                                if (_value == null) {
                                  ...
                                  print(token);
                                  SetAddressRequest request = SetAddressRequest(
                                      latitude: snapshot.data[_value].latitude,
                                      longitude: snapshot.data[_value].longitude);

                                  int sid = snapshot.data[_value].id;
                                  String url = "user/set-delvery-address/";
                                  var res = await NetworkUtil()
                                      .post(url: url + sid.toString(), body: jsonEncode(request), token: token);
  ...
  getAddressList() async {
    ...
    if (response.status == 200) {
      ...
      CommonUtils.showToast(
          msg: "Please Select Delivery Address", bgColor: AppColor.darkThemeBlueColor, textColor: Colors.white);
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => ChooseLocation(
